...
Why: the model is not self-evident from the UI — pools sitting between collections and lists, and orderings being derived from comparisons rather than authored, are both non-obvious. A new user currently has to infer the mental model from the interface alone.
