...
- [ ] T008 [artifacts: infrastructure, datamodel] Provision a Supabase project; add env/config loading and a single DB connection/client module (constructed at the entry, imported elsewhere — Principle XV). No secrets committed.
- [ ] T009 [artifacts: datamodel] Write the initial migration creating `User`, `Game`, `Collection`, `CollectionItem`, `List`, `Comparison`, `ListEntry` and all indexes in `datamodel.md`. Test: migration applies to a clean database and the resulting schema (columns, unique `Game.bgg_id`, FKs) matches expectations.
...
- [ ] T021 [artifacts: ui, infrastructure] Auth UI (sign in/up/out) wired to Supabase Auth. Test the flow; WCAG 2.1 AA (axe + keyboard).
