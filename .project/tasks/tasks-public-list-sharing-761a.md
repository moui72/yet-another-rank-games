...
- [ ] T004 [artifacts: ui] [parallel] Add an enable/disable share toggle and a copy-link affordance to the List result & export view, wired to `is_shared` and displaying the `/share/[token]` URL once enabled. Write a failing test first covering: toggling on shows a copyable link; toggling off keeps the link functional per the non-revocable model but updates the UI's own displayed state accordingly (Principle I).
