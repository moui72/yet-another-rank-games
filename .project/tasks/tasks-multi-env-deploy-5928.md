...
- [x] T007 [artifacts: infrastructure] [partial: workflow + rollback docs written; blocked on human GCP/GitHub steps before it can run for real — see below] Production deploy workflow `deploy-production.yml`: on push to `production`, behind the `production` Environment approval gate, run prod migrations then deploy **the same SHA image** already on staging (resolve the image by the `production` tip commit SHA — no rebuild) to production Cloud Run via WIF (`envs/production` outputs). Document and **exercise rollback** as a Cloud Run traffic shift to the prior revision (`gcloud run services update-traffic`). Add the Production Annotations from the plan (single-region, Supabase free tier, RLS-off) at the relevant points.
  ...
  - Rollback documented in `infra/terraform/README.md` (new "Rollback
    ...
    — see blockers below.
  - **BLOCKED (2026-07-13) — now resolved except one manual step:**
    1. **Cross-project Artifact Registry grant** — DONE. The user explicitly
       authorized this and it was applied:
       ...
       --role="roles/artifactregistry.reader"`. Confirmed in the returned
       IAM policy binding.
    2. **GitHub `production` Environment vars/secrets** — mostly DONE, user
       explicitly authorized piping secrets. Set this run:
       - vars: `GCP_WIF_PROVIDER`, `GCP_DEPLOYER_SA`,
         `PRODUCTION_SUPABASE_URL`, `PRODUCTION_SUPABASE_PUBLISHABLE_KEY`
         (all via `gh variable set --env production`).
       - secrets: `PRODUCTION_BILLING_ACCOUNT` (literal value, resolved via
         `gcloud billing projects describe`) and `SUPABASE_DB_PASSWORD`
         (piped directly from `gcloud secrets versions access latest
         --secret=db-password --project=yarg-production-cwqd` into
         `gh secret set`, never displayed).
       - **Still open:** `SUPABASE_ACCESS_TOKEN` — the account owner's
         personal Supabase token. Can't be copied from `staging`'s existing
         secret (`gh secret list` never exposes values) or generated by an
         agent. The user chose to set this one themselves via
         `gh secret set SUPABASE_ACCESS_TOKEN --env production`.
    3. **Exercise the rollback** — still requires a real production deploy
       to be live first (so there's a "prior revision" to shift back to).
       Once `SUPABASE_ACCESS_TOKEN` is set: push a commit through
       `promote-to-production.yml`, confirm `deploy-production.yml` goes
       green, then run the `gcloud run services update-traffic` commands in
