...
- [x] T002 [artifacts: ui] Wrap `copyShareLink`'s `navigator.clipboard.writeText` call in a try/catch; on rejection, set a failure-state flag/message the template renders (reusing the existing copy-status UI slot used for the success "Copied!" case) so the user sees the write failed. Make T001's test pass.
