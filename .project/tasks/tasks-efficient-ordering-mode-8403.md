...
- [x] T023 Remove `manual` from the `RankingMethod` type in `src/lib/types/entities.ts` and fix the resulting type errors. Full suite plus typecheck must be green; this task is complete only when nothing references the retired value.
