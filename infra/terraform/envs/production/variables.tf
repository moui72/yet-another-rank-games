...
variable "web_min_instances" {
  type    = number
  default = 1 # production keeps one warm instance (no cold starts)
}
...
variable "enable_kill_switch" {
  type    = bool
  default = true # NOTE: tripping this on production is a full outage (billing off → Cloud Run stops). Set false to alert-only.
}
