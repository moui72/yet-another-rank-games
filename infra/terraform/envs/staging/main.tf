terraform {
  ...
  backend "gcs" {
    bucket = "yarg-staging-zbch-tfstate"
    prefix = "staging"
  }
}
...
module "environment" {
  ...
  region            = var.region
  env               = "staging"
  web_min_instances = var.web_min_instances # staging: scale to zero
...
output "wif_provider" {
  description = "Set as workload_identity_provider in the staging deploy workflow."
